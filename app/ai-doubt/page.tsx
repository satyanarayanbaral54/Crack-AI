...
function AiDoubtContent() {
  ...
  return (
    <main className="flex min-h-svh bg-slate-100 px-2 py-2 text-slate-950 dark:bg-slate-950 dark:text-white sm:px-6 sm:py-6">
      <section className="mx-auto flex h-[calc(100svh-1rem)] w-[calc(100vw-1rem)] max-w-6xl flex-col overflow-hidden rounded-2xl border border-white/70 bg-white shadow-glow dark:border-white/10 dark:bg-slate-900 sm:h-[calc(100svh-3rem)] sm:w-full sm:rounded-[1.5rem]">
        <header className="flex flex-col gap-3 border-b border-slate-200 px-3 py-3 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-teal-600 dark:text-teal-300">
              AI Doubt Solver
            </p>
            <h1 className="mt-2 break-words text-xl font-semibold tracking-normal sm:text-3xl">
              {exam} Chat
            </h1>
          </div>

          <div className="grid w-full grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-2 sm:flex sm:w-auto sm:flex-row">
            <button
              type="button"
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 px-3 py-4 dark:bg-slate-950 sm:px-6 sm:py-5">
          <div className="mx-auto flex max-w-4xl flex-col gap-5">
            {quizError ? <AuthMessage message={quizError} type="error" /> : null}

            {messages.length === 0 ? (
              <div className="flex min-h-[42vh] items-center justify-center">
                <div className="max-w-xl break-words text-center">
                  <Image
            ...
            {messages.map((message) =>
              message.role === "student" ? (
                <div key={message.id} className="flex justify-end">
                  <div className="max-w-[92%] break-words rounded-2xl rounded-br-md bg-teal-600 px-4 py-3 text-sm leading-6 text-white shadow-lg shadow-teal-600/15 sm:max-w-[70%]">
                    {message.question}
                  </div>
                </div>
              ) : (
                <div key={message.id} className="flex justify-start">
                  <article className="w-full max-w-3xl break-words rounded-2xl rounded-bl-md border border-slate-200 bg-white p-3 shadow-sm dark:border-white/10 dark:bg-slate-900 sm:p-5">
              ...
              <section className="rounded-2xl border border-teal-200 bg-white p-4 shadow-sm dark:border-teal-500/30 dark:bg-slate-900 sm:p-5">
                <div className="flex flex-col gap-3 border-b border-slate-200 pb-4 dark:border-white/10 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <h2 className="mt-2 break-words text-xl font-semibold tracking-normal">
                      {quizTopic}
                    </h2>
                  </div>
        ...
        >
          <div className="mx-auto flex max-w-4xl gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-2 focus-within:border-teal-500 focus-within:ring-4 focus-within:ring-teal-500/10 dark:border-white/10 dark:bg-slate-950 sm:gap-3">
            <textarea
              value={question}
              ...
              className="max-h-36 min-h-12 min-w-0 flex-1 resize-none bg-transparent px-3 py-3 text-sm text-slate-950 outline-none placeholder:text-slate-400 dark:text-white dark:placeholder:text-slate-500"
            />
            <button
              type="submit"
              disabled={loading || !question.trim()}
              className="flex h-12 w-14 shrink-0 items-center justify-center rounded-xl bg-teal-600 px-0 text-sm font-semibold text-white shadow-lg shadow-teal-600/20 transition hover:bg-teal-500 focus:outline-none focus:ring-4 focus:ring-teal-500/30 disabled:cursor-not-allowed disabled:opacity-70 sm:w-auto sm:min-w-24 sm:px-5"
            >
              {loading ? (
                <span
