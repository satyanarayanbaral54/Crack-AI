...
export default function WelcomeScreen({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative flex w-full max-w-[420px] flex-col items-center px-6 transition-all duration-700 ${
          fadeOut ? "translate-y-3 scale-95 opacity-0" : "translate-y-0 scale-100 opacity-100"
        }`}
      >
        <div className="relative flex h-72 w-72 items-center justify-center sm:h-80 sm:w-80">
          <div className="logo-aura absolute h-56 w-56 rounded-full bg-teal-300/20 blur-3xl sm:h-64 sm:w-64" />
          <div className="logo-ring absolute h-64 w-64 rounded-full border border-white/10 sm:h-72 sm:w-72">
          ...
          <div className="logo-shell relative rounded-[28px] bg-white/[0.03] p-5 shadow-[0_26px_80px_rgba(0,0,0,0.45)] backdrop-blur-md">
            <Image
              ...
              height={240}
              priority
              className="h-auto w-[190px] drop-shadow-[0_0_36px_rgba(45,212,191,0.38)] sm:w-[230px]"
            />
          </div>
        </div>
